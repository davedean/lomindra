...
// Note: Compile with sync_lib.swift:
...
func loadConfig() throws -> Config {
    ...
    if let envListMap = ProcessInfo.processInfo.environment["LIST_MAP_PATH"], !envListMap.isEmpty {
        listMapPath = envListMap
    }
    var syncAllLists = parseBool(apple["sync_all_lists"])
    if let envSyncAll = ProcessInfo.processInfo.environment["SYNC_ALL_LISTS"] {
        syncAllLists = parseBool(envSyncAll)
    }
    var syncDbPath = apple["sync_db_path"] ?? "sync.db"
    if let envDbPath = ProcessInfo.processInfo.environment["SYNC_DB_PATH"], !envDbPath.isEmpty {
...
func openSyncDb(path: String) throws -> OpaquePointer? {
    ...
    if sqlite3_exec(db, createSql, nil, nil, nil) != SQLITE_OK {
        defer { sqlite3_close(db) }
        let message = String(cString: sqlite3_errmsg(db))
        throw NSError(domain: "sqlite", code: 2, userInfo: [NSLocalizedDescriptionKey: "Create table failed: \(message)"])
    }
...
enum ConflictResolutionPolicy: String {
    case none
    case reminders
    case vikunja
...
func conflictResolutionPolicy(from value: String?) -> ConflictResolutionPolicy {
    guard let value = value?.lowercased(), !value.isEmpty else {
        return .none
    }
    return ConflictResolutionPolicy(rawValue: value) ?? .none
}
...
func runSync() {
    do {
        ...
        let resolveConflicts = conflictResolutionPolicy(from: argumentValue("--resolve-conflicts"))
        ...
        for mapping in mappings {
            ...
            if apply {
                if !plan.conflicts.isEmpty && !allowConflicts && resolveConflicts == .none {
                    fputs("Conflicts detected (\(plan.conflicts.count)). Re-run with --allow-conflicts or --resolve-conflicts=reminders|vikunja|last-write-wins.\n", stderr)
                    exit(3)
                }
